<?php
// ============================================================
//  edit_item.php — Edit an Existing Item (self-contained)
//  Lets users update item details and attach/replace a photo
// ============================================================
...
$errors  = [];
$success = false;
// Pre-fill form fields with current item data; $_POST overrides on validation failure
$old = $item;
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    // ── Photo handling ─────────────────────────────────────
    // Default: keep the existing photo unless replaced or removed
    ...
    if (isset($_FILES['photo']) && $_FILES['photo']['error'] !== UPLOAD_ERR_NO_FILE) {
        ...
        // User explicitly chose to remove the photo without replacing it
        if (!empty($item['photo_path']) && file_exists($item['photo_path'])) {
            @unlink($item['photo_path']);
        }
        $photo_path = null;
    ...
    if (empty($errors)) {
        ...
        $stmt->bind_param('iisssssi',
            $category_id, $location_id, $item_type, $item_name,
            $description, $photo_path, $date_reported, $item_id
        );
        if ($stmt->execute()) {
            $success = true;
            // Refresh $item / $old with the latest saved values
...
.current-photo-wrap {
  ...
  background: #f8fafc; border: 1.5px solid var(--clf-border);
  border-radius: var(--clf-radius-sm); padding: .85rem 1rem;
  margin-bottom: .85rem;
}
.current-photo-wrap img {
  width: 72px; height: 72px; object-fit: cover; border-radius: 8px; flex-shrink: 0;
}
.current-photo-info { flex: 1; font-size: .82rem; color: var(--clf-muted); }
.current-photo-info strong { display: block; color: #0f172a; font-size: .85rem; margin-bottom: .15rem; }
.btn-remove-photo {
  background: #fef2f2; color: #dc2626; border: 1px solid #fecaca;
  border-radius: var(--clf-radius-sm); font-size: .78rem; font-weight: 600;
  padding: .4rem .8rem; cursor: pointer; white-space: nowrap;
  font-family: 'DM Sans', sans-serif;
}
.btn-remove-photo:hover { background: #fee2e2; }
.btn-remove-photo.undo { background: #eff6ff; color: #1a56e8; border-color: #bfdbfe; }
...
<main class="clf-main">
  <div class="page-header">
    <h1>Edit Item</h1>
    <p>Update item details or attach a photo for item #<?= $item_id ?>.</p>
  </div>
    ...
    <div class="col-lg-8 col-xl-7">
      ...
      <?php if ($success): ?>
        <div class="clf-alert-success">
          ...
          <div>
            ...
            <a href="view_item.php?item_id=<?= $item_id ?>" class="ms-2 fw-semibold" style="color:#166534;">View item →</a>
          </div>
        </div>
      <?php endif; ?>
      ...
      <div class="clf-card">
        ...
        <div class="clf-card-body">
          <form id="editItemForm" method="POST" action="edit_item.php?item_id=<?= $item_id ?>"
            ...
            <div class="mb-4">
              ...
              <?php if (!empty($item['photo_path']) && file_exists($item['photo_path'])): ?>
                ...
                <div class="current-photo-wrap" id="currentPhotoWrap">
                  <img src="<?= e($item['photo_path']) ?>" alt="Current photo">
                  <div class="current-photo-info">
                    <strong>Current photo</strong>
                    Upload a new photo below to replace it, or remove it entirely.
                  </div>
            ...
            <div class="d-flex gap-2">
              ...
              <a href="view_item.php?item_id=<?= $item_id ?>" class="btn-clf-secondary">Cancel</a>
            </div>
...
function showPreview(file) {
  ...
  // If a new file is chosen, cancel any pending "remove" request
  const removeInput = document.getElementById('removePhotoInput');
  if (removeInput) removeInput.value = '0';
  const wrapEl = document.getElementById('currentPhotoWrap');
